...
def _infer_rhns_course(sheet_name: str, class_title: str, courses: list[str]) -> str:
    ...
    # Health and Movement Science does not have a 2021–2025 five-year scaling history
    # in the current model, so do not silently substitute PDH&PE.
    if 'health and movement science' in t:
        return ''
...
def _build_rhns_student_reports(records: pd.DataFrame, prediction_date: date, year_group: str = 'Year 11'):
    ...
    for student_key, grp in records.groupby('Student key', sort=True):
        ...
        # Any missing/invalid prediction in an included subject blocks that student's report.
        valid_mask = grp.apply(
            lambda r: _valid_prediction_code(r.get('Course', ''), r.get('Predicted band', '')),
            axis=1,
        )
        bad_rows = grp[~valid_mask]
        if not bad_rows.empty:
            details = []
            for _, bad in bad_rows.iterrows():
                course = str(bad.get('Course', ''))
                code = str(bad.get('Predicted band', '') or 'blank')
                expected = 'E4A–E1C' if _is_extension_course(course) else '6A–2C'
                details.append(f'{course} ({code}; expected {expected})')
            errors.append({
                'Student': student_name,
                'Issue': 'Missing or invalid HSC prediction: ' + '; '.join(details),
            })
            continue

        # Collapse duplicate appearances of the same course, but reject conflicting band codes.
        course_rows = []
        conflict_courses = []
        for course, cgrp in grp.groupby('Course', sort=False):
            bands = sorted(set(cgrp['Predicted band'].tolist()))
            if len(bands) > 1:
                conflict_courses.append(str(course))
                continue
            course_rows.append((str(course), bands[0]))

        if conflict_courses:
            errors.append({
                'Student': student_name,
                'Issue': 'Conflicting predictions for: ' + ', '.join(sorted(conflict_courses)),
            })
            continue
...
def render_bulk_section(courses: list[str]):
    ...
    can_calculate = not included.empty and invalid.empty

...
# Health and Movement Science has no historical scaling rows in the 2021–2025
# dataset. At RHNS we use PDH&PE as an explicit provisional proxy until genuine
# HMS scaling data can replace it.
HMS_COURSE = 'Health and Movement Science'
HMS_PROXY_COURSE = 'PDH&PE'
SCALING_PROXY = {HMS_COURSE: HMS_PROXY_COURSE}
...
def available_courses() -> list[str]:
    courses = set(SCALING.course.unique().tolist())
    courses.update(SCALING_PROXY.keys())
    return sorted(courses)


def scaling_source_course(course: str) -> str:
    """Return the historical course whose UAC scaling curve is used."""
    return SCALING_PROXY.get(course, course)


def is_provisional_proxy(course: str) -> bool:
    return course in SCALING_PROXY


def _curve(course: str, year: int):
    source_course = scaling_source_course(course)
    r=SCALING[(SCALING.course==source_course)&(SCALING.year==year)]
    if r.empty:
        return None
    r=r.iloc[0]
...
def estimate(rows: list[dict], sensitivity: float=2.0) -> dict:
    ...
    for r in clean:
        ...
        proxy = is_provisional_proxy(r['course'])
        subject.append({
            ...
            'scaling_source_course': scaling_source_course(r['course']),
            'provisional_proxy': proxy,
